    ...
    public class EFRepositoryFactories : IDisposable
    {
        ...
        private static IDictionary<Type, Func<IDbContext, object>> GetCustomFactories()
        {
            return new Dictionary<Type, Func<IDbContext, object>>
            {
                ...
                {typeof (IScoreRepository), dbContext => new ScoreRepository(dbContext)},
                {typeof (ITrackRepository), dbContext => new TrackRepository(dbContext)},
                {typeof (IBasketRepository), dbContext => new BasketRepository(dbContext)},
                {typeof (IPlayerInGameRepository), dbContext => new PlayerInGameRepository(dbContext)},
